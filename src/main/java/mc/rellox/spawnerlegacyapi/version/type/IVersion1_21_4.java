...
public class IVersion1_21_4 implements IVersion {
	...
	private static final Invoker<?> _datawatcher = RF.order(_entity, "ar");
	...
	public Object hologram(Location l, String name) {
		...
		RF.order(d, "u", boolean.class).invoke(true); // marker
		RF.order(d, "k", boolean.class).invoke(true); // invisible
		name(d, name);
		RF.order(d, "o", boolean.class).invoke(true); // set custom name visible
