...
public interface IVariables {
    ...
    static IVariables with(String key, Object value) {
        var objects = new Object[] {key, value};
        return () -> objects;
    }

    static IVariables with(Object... pairs) {
        if(pairs == null || pairs.length == 0) return empty;
        if(pairs.length % 2 != 0)
            throw new IllegalArgumentException("Variable pair length must be even");
        return () -> pairs;
    }
